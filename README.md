...
This project implements a coordinate-based tracking system that computes the
closest point, offset, and station of a user-provided position relative to
a predefined polyline path loaded from a CSV file.

It consists of an Angular frontend for visualization and interaction, and a
.NET 8 backend responsible for geometry calculations and API access.

## Demo
A demo is available at https://chatter-api.space/

## Overall Project Structure
The project is divided into two logical components: tracker-frontend and tracker-backend.

- [tracker-frontend](./tracker-frontend/) contains the Angular-based [frontend](#frontend) application.
- [tracker-backend](./tracker-backend/) contains the .NET 8 [backend](#backend), responsible for both the API and application hosting.

The system is designed to operate as a single, integrated unit, where the .NET backend serves the REST API consumed by the frontend as well as the static files generated by the Angular build. This unified setup simplifies deployment and ensures tight integration between frontend and backend.
## Frontend
...
- If t > 1: it is clamped to 1, and the end point (B) is selected as the closest point.
- If t < 0: it is clamped to 0, and the start point (A) is selected as the closest point.
